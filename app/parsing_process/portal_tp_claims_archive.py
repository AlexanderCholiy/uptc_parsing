...
PARSING_TIMER: int = 180  # Сайт очень медленный
MONTHS: dict = {
    'января': 1,
    'февраля': 2,
    'марта': 3,
...
def portal_tp_claims_archive(login: str, password: str, *args) -> DataFrame:
    ...
    portal_tp_logger.debug('Запуск сбора архивных заявок')
    ...
    wait.until(
        EC.element_to_be_clickable(
            (
                By.XPATH,
                "//li[contains(@class, 'ui-tabs-header') "
                "and contains(., 'Архив')]/a"
            )
        )
    ).click()

    portal_tp_logger.debug('Перешли на страницу архивных заявок')
    ...
    while True:
        ...
        portal_tp_logger.debug(f'Получили список дат заявок ({scroll_number})')
